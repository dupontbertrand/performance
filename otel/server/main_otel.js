...
Meteor.startup(async () => {
  ...
  Meteor.methods({
    async 'links.insert'(traceContext = {}) {
      ...
      const startTime = Date.now(); // note: latency already registred in tracer

      addEvent('validation.start', { sessionId });

      await withSpan('links.insert', 'promise-time-sleep', async () => {
        //wait 1s
      ...
      await withSpan('links.insert', 'Link-mongo-insertion', async () => {
        try {
